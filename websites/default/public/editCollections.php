...
$title = 'Fotheby\'s - Edit Collections';
require '../head.php';
require '../nav.php';
...
if(isset($_SESSION['adminloggedin'])) {
    ...
    if (isset($_POST['submit'])) {
        $stmt = $pdo-> prepare('UPDATE collections SET name=:newName WHERE name=:oldName');

        unset($_POST['submit']);
        $stmt ->execute($_POST);

        //this is to let the user know that their changes have been made
        echo '<p>Collection Edited</p>';
    }
    ...
    $selectCol = $pdo->prepare('SELECT * FROM collections WHERE name = :name');
        $value = [
            'name' => $_GET['name']
        ];

        $selectCol -> execute($value);
        $collections = $selectCol->fetch();  
?>
    <form action="editCollections.php" method= "POST" enctype = "multipart/form-data">
        <p>Edit Collections:</p>

        <label>Name</label> <input type="text" name = "newName" value= "<?php echo $collections['name']; ?>" required />
        
        <input type = "hidden" name = "oldName" value = "<?php echo $collections['name']; ?>" required/>
