<?php
    ...
    if (isset($_GET['periodId'])) {
        ...
        echo '<h3> Auction Period: ' . $_GET['periodId'] . ' </h3>';
        ...
        $selectPeriod = $pdo->prepare('SELECT * FROM article WHERE auctionPeriod = :auctionPeriod');
        $value = [
            'auctionPeriod' => $_GET['periodId'],
        ];

        $selectPeriod -> execute($value);
        ...
        foreach($selectPeriod->fetchAll() as $row) {
            echo '<a class="articleLink" href="periodPages.php?periodId=' . $row['auctionPeriod'] . '">' 
